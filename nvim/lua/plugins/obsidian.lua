return {
					...
					if not string.match(line, checkbox_pattern) then
						...
						for i, check_char in require("obsidian.itertools").enumerate(checkboxes) do
							if
								string.match(
									line,
									"^%s*- %[" .. obsidian.util.escape_magic_characters(check_char) .. "%].*"
								)
							then
								if i == #checkboxes then
									i = 0
								end
								line = require("obsidian").util.string_replace(
									line,
									"- [" .. check_char .. "]",
									"- [" .. checkboxes[i + 1] .. "]",
									1
	...
	config = function(_, opts)
		...
		for i = -9, 9 do
			vim.api.nvim_set_keymap("n", "od" .. i, "", {
				noremap = true,
				callback = (function(num)
					return function()
						obsidian_today_with_number(num)
					end
				end)(i),
				silent = true,
			})
